...
public:
    ...
    void start();
    void enqueue_request(TransactionId tid, std::vector<uint8_t> request,
...
private:
    struct ReadState {
        ...
        void read_next() {
            ...
            boost::asio::async_read(
                self->device_socket_, boost::asio::buffer(*header_buf),
                [self, this](boost::system::error_code ec, std::size_t) mutable {
                    ...
                    boost::asio::async_read(
                        ...
                        [self, this](boost::system::error_code ec, std::size_t bytes_read) mutable {
                            ...
                            uint16_t tid = ((*current_response)[0] << 8) | ((*current_response)[1]);
                            auto it = std::find(tids.begin(), tids.end(), tid);
                            if (it != tids.end()) {
                                tids.erase(it);
    ...
    boost::posix_time::time_duration timeout_ = boost::posix_time::milliseconds(300);

    void async_write_read(
        uint16_t request_count, std::vector<uint16_t> tids, const std::vector<uint8_t>& data,
        std::function<void(boost::system::error_code, std::vector<uint8_t>)> callback);
    void async_read_n_responses(
